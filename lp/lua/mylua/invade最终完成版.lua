...
传说级入侵者2 = {deadFlag = false , canBattleFlag = true , info = "3|0,20003,43,7||0|||||0|10007|||||||||"} ,
...
function invadeBossNPCTalk(_me,_tome)
	...
	if not bossDeadFlag then
		if bossCanBattleFlag then
			...
			local msg = "我正忙着对付别的小虫子呢！哦，还有他们的尸体。。"
...
function invadeBossNPCWindowTalk(npc,player,Seqno,Select,Data)
	...
	if Select ~= 2 then
		if Seqno == 1000 then
			...
			print("bos Name = " .. playerAndBoss[player]);
...
function battleWinEvent(_battlePtr,_chrPtr)
	...
	if winSide == 0 then
		NLG.TalkToCli(-1,-1,"异族入侵战役——" .. bossName .. "已经被击败！",%颜色_红色%);
