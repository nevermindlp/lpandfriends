...
function Entry (b)
		petFusionData[b.id] = {lv = b.lv , name = b.name , feature = b.feature , part = b.part , eggId = b.eggId};
end
dofile("/gmsv/lua/Module/data.lua")
...
function FeedEggNPC()
	if FeedEggNPC == nil then
		...
		Char.SetData(FeedEggNPC,%对象_原名%,"宠物融合npc")
...
function FeedEggNPCWindowTalk(npc,player,Seqno,Select,Data)
	...
	if Select ~= 2 then
		--获取第一栏道具
		if Seqno == 710 then
			local item = Char.GetItemIndex(player,1);
			--判断道具是否为蛋可以通过名称或ID区间
			--...
			...
			if findSecondFlag or findFinalFlag then
				...
				eggCreateTime = string.sub(str, 1, pos - 1);
			end
				...
				if findSecondFlag then
					...
					for k,v in pairs(petFusionData) do
						if v.eggId == targetEggId then
							...
							Char.AddPet(player,targetPetId);
						end
					end
					--删除原来的道具，将新道具（蛋）给玩家,并设置新蛋的时间和等级
					Item.Kill(player, item, 1);
...
function PetFusionNPCWindowTalk(npc,player,Seqno,Select,Data)
	...
	if Select ~= 2 then
		if Seqno == 710 then
			...
			if pet1Lve < 30 or pet2Lve < 30 then
